...
function validatePNP(){
	console.log("validating using the PNP layout");
	$('.layoutType').html("<h3>Using PNP Layout</h3>");
	...
	for(k=0; k < lines.length; k++){
		...
		switch(lineType){
			case "00":
				PNPHeader(currentLine);
				break;
			case "01":
				PNPBody01(currentLine);
				break;
			case "02":
				PNPBody02(currentLine);
				break;
			case "03":
				PNPBody03(currentLine);
				break;
			case "99":
				PNPTrailer(currentLine, lines.length);
				break
			default:
...
function PNPHeader(line){
	console.log("PNPHeader");
	//Credencial
	numberValidator(line,0, 4, false, 'Credencial', '');
	//Identificação do Programa 
	stringValidator(line,4, 3, false, 'Identificação do Programa ', 'PNP');
	//Ano Safra
	numberValidator(line,7, 6, false, 'Ano Safra', '');
	PNPAnoSafra = line.substr(7,6);
...
function PNPBody01(line){
	console.log("PNPBody01");
	//Credencial
	numberValidator(line,0, 4, false, 'Credencial', '');
	//Identificador do Programa
	stringValidator(line,4, 3, false, 'Identificador do Programa', 'PNP');
	//Ano Safra
	numberValidator(line,7, 6, false, 'Ano Safra', PNPAnoSafra);
...
function PNPBody02(line){
	console.log("PNPBody02");
...
function PNPBody03(line){
	console.log("PNPBody03");
	
	//Credencial
	numberValidator(line,0, 4, false, 'Credencial', '');
	//Tipo de Solicitação
	stringValidator(line,4, 3, false, 'Tipo de Solicitação', 'PNP');
	//Ano Safra
	numberValidator(line,7, 6, false, 'Ano Safra', PNPAnoSafra);
	//Identificação do Movimento
	numberValidator(line,13, 5, false, 'Identificação do Movimento', '');
	...
	stringValidator(line,29, 3, false, 'Código do tipo da informação complementar', '001,002,003,004,027,028');
	//Valor da informação complementar
	validarPNPBody03ValorInformacaoComplementar(line, line.substr(29,3), 'Valor da informação Complementar');	
}

function validarPNPBody03ValorInformacaoComplementar(line, value, field){
	switch(value){
		case '001':
...
function PNPTrailer(line, linesLength){
	console.log("PNPTrailer");
	//Credencial
	numberValidator(line,0, 4, false, 'Credencial', '');
	//Identificador do Programa
	stringValidator(line,4, 3, false, 'Identificador do Programa', 'PNP');
